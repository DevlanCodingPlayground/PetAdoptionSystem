...
/* Add Pet Adopters */
if (isset($_POST['Add_Pet_Adopter'])) {
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        if (mysqli_query($mysqli, $auth_sql)) {
            ...
            if (mysqli_query($mysqli, $adopter_sql)) {
                $success = "Pet adopter account created";
            }
        } else {
...
/* Update Pet Adopters */
if (isset($_POST['Update_Pet_Adopters'])) {
    $pet_adopter_name = mysqli_real_escape_string($mysqli, $_POST['pet_adopter_name']);
    $pet_adopter_email = mysqli_real_escape_string($mysqli, $_POST['pet_adopter_email']);
    $pet_adopter_phone_number = mysqli_real_escape_string($mysqli, $_POST['pet_adopter_phone_number']);
    $pet_adopter_address = mysqli_real_escape_string($mysqli, $_POST['pet_adopter_address']);
    ...
    /* Persist */
    $update_sql = "UPDATE pet_adopter SET pet_adopter_name = '{$pet_adopter_name}', pet_adopter_email = '{$pet_adopter_email}',
    pet_adopter_phone_number = '{$pet_adopter_phone_number}', pet_adopter_address = '{$pet_adopter_address}' 
    WHERE pet_adopter_id = '{$pet_adopter_id}'";

    if (mysqli_query($mysqli, $update_sql)) {
        $success = "Pet adopters account updated";
    } else {
